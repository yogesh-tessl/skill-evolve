---
name: skill-evolve
description: "通过观察输出、提炼错误模式、针对性重写指令、验证改进效果的演进式循环来改进 skill 质量。当用户说'改进这个 skill'、'这个 skill 效果不好'、'优化 skill'、'skill 不够好'、'迭代 skill'、'skill-evolve'、'/evolve'时触发。也适用于用户拿着一个 skill 的输出结果说'不满意'、'有问题'、'效果差'的场景。不要等用户明确说'演进'——只要他们在讨论一个 skill 的质量问题，就应该启用。"
---

# Skill Evolve — 演进式 Skill 改进

用户会给你一个已有的 skill（或指向一个 skill 的路径），通过**观察→总结→改进→验证**的循环，系统性地提升它的质量。
...
**不要逐个修 bug，要找共性。** 回顾所有 prompt 的观察记录，问自己：
...
2 条模式比 8 个补丁更有价值——模式能覆盖未来的新 prompt。

### 第四步：改写——JIT 原则，每轮只改一件事（15-30 分钟）

从错误模式表中取优先级最高的 1-2 个模式，针对性改写。不要一次性重写整个 skill。改写时遵循：
...
每轮的 `observations.md`、`patterns.md`、`changes.md` 是下一轮改进的上下文。Round 2 开始时，先读 Round 1 的全部笔记。

当积累了 3 轮以上的改进笔记，做一次递归压缩。详见 [references/bootstrap.md](references/bootstrap.md)。
...
如果用户带着一个具体问题来（问题明确、改动局部），走快速路径而非完整五步循环。关于快速模式、skill-creator 分工、非收敛处理等场景，详见 [references/special-scenarios.md](references/special-scenarios.md)。
